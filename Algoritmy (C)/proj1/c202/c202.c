...
*/
    ...
    if (s == NULL) {
        ...
        s->top = -1; // Nastavime vrchol tak, aby indikoval prazdny zasobnik
    }
}
...
*/
    return (s->top == -1); // Pokud je vrchol na -1, zasobnik je prazdny
}
...
*/
    return (s->top == STACK_SIZE-1); // Pokud je vrchol na poslednim indexu, je zasobnik plny
}
...
*/
    ...
    if (stackEmpty(s)) {
        ...
        *c = s->arr[s->top]; // Vratime znak na indexu vrcholu pres ukazatel
    }
}
...
** 
*/
    /* Pokud je zasobnik prazdny, nic nedelame */
    if (!stackEmpty(s)) {
        s->top--; // Posuneme ukazatel vrcholu, tim znak "jakoby" vymazeme
    }
}
...
*/
    ...
    if (stackFull(s)) {
        ...
        s->arr[++s->top] = c; // Nejprve posuneme vrchol o 1 vys, tim udelame misto a vlozime na to misto znak
    }
}
